...
1. **Adaptive throttle** (`applescript/throttle.ts`)
   - Module-level rolling window of last 10 AppleScript call latencies, time-decayed (samples older than 5 min are excluded from classification)
   - Classifies bridge state from the **median** of recent samples: `healthy` (<500ms) / `normal` (<1000ms) / `stressed` (<2000ms) / `degraded` (≥2000ms)
   - **Median, not p95** — an earlier version used `p95()` with `floor(N * 0.95)` which for N ≤ 20 is mathematically equivalent to `max()`. One slow call (e.g. a 13s cold-start search) then pinned the metric to its own value for the full 5-min decay, refusing every subsequent operation regardless of how many fast calls followed. Median is correct for "is the bridge typically slow right now?" — one outlier moves it by zero; sustained degradation moves it sharply.
...
- **Date-first search strategy (added 2026-05-14):** when `after`/`before` is provided, `searchMessages` skips the subject predicate in the AppleScript `whose` clause entirely and runs a date-only `whose time received ≥ afterDate [and ≤ beforeDate]`. The walk extracts id/subject/sender/date for every match (capped at `DATE_FIRST_MAX_SCAN = 500` to bound the budget at ~15s), and `applySubjectSenderFilter` in TypeScript then filters for query containment against `subject`, `sender`, and `senderAddress`. This eliminates the failure mode observed 2026-05-14 in the temp project: two consecutive `search_emails(query="Gmail storage", after="2026-05-01")` calls each hit the 65s subprocess timeout (-1712 AppleEvent timeouts) before the brake fired. Outlook's `whose subject contains X AND time received ≥ Y` is mysteriously slower than `whose time received ≥ Y` even though the latter superset gets walked anyway — the date predicate alone is something Outlook can bound efficiently, but adding the subject substring forces a full-folder scan. The date-first path also skips Phase 2 (sender loop) entirely since sender data is already on every row from the single pass.
- **Safety brake is reactive, not predictive:** the `OutlookBridgeStressedError` brake only fires once the rolling median latency window crosses 2000ms — meaning brief slow patches don't trigger refusals; sustained degradation does. Two predictive measures close the gap for the "first heavy call" problem:
