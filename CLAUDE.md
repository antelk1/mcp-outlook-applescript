...
- **Safety brake is reactive, not predictive:** the `OutlookBridgeStressedError` brake only fires once the rolling p95 latency window crosses 2000ms — meaning the FIRST heavy call always gets through, regardless of how dangerous it is. To close the gap, the unscoped-search predictive guard now refuses `searchEmails(query, ...)` (no `folder_id`) when the cached total folder message count exceeds 50,000, surfacing `OUTLOOK_QUERY_REFUSED` with a structured suggestion to pass `folder_id`. This is INPUT-based, not bridge-state-based: it fires even on a freshly-restarted healthy bridge if the query would do mailbox-wide scanning.
