...
interface DefaultSearchFolders {
    readonly inboxId: number;
    readonly sentItemsId: number | null;
}
...
export class AppleScriptRepository implements IWriteableRepository {
    ...
    private readonly defaultSearchFoldersCache = new TtlCache<DefaultSearchFolders>(DEFAULT_SEARCH_FOLDERS_TTL_MS);
     ...
     */
    private getDefaultSearchFolders(): DefaultSearchFolders {
        ...
        const resolved: DefaultSearchFolders = {
     ...
     */
    private gateLargeFolderSearch(folderId: number, after: string | undefined, before: string | undefined, operation: string): void {
        ...
        if (count > LARGE_FOLDER_THRESHOLD) {
            throw new OutlookQueryRefusedError(
                operation,
                `folder "${folder?.name ?? folderId}" contains ${count.toLocaleString()} messages (>${LARGE_FOLDER_THRESHOLD.toLocaleString()} threshold) and unbounded search would risk destabilizing the Outlook AppleScript bridge`,
                `Pass \`after\` and/or \`before\` (ISO 8601) to bound the search by date — e.g., \`after: "2025-01-01T00:00:00Z"\` to search the last year. Date-bounded searches at this size are safe.`,
            );
        }
    }
