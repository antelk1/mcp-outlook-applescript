 ...
 */
function parseAttachmentDetails(value) {
    if (value === undefined || value === '' || value === DELIMITERS.NULL) {
        return [];
    }
 ...
 */
export function parseFolders(output) {
 ...
 */
export function parseEmails(output) {
    const records = parseRawOutput(output);
    return records.map((r) => ({
        id: parseNumber(r['id']),
 ...
 */
export function parseEmail(output) {
 ...
 */
export function parseCalendars(output) {
    const records = parseRawOutput(output);
    return records.map((r) => ({
        id: parseNumber(r['id']),
        name: parseString(r['name']),
 ...
 */
export function parseContact(output) {
 ...
 */
export function parseTask(output) {
 ...
 */
export function parseAccounts(output) {
    const records = parseRawOutput(output);
    return records.map((r) => ({
        id: parseNumber(r['id']),
        name: parseString(r['name']),
 ...
 */
export function parseDefaultAccountId(output) {
    const trimmed = output.trim();
 ...
 */
export function parseCreateEventResult(output) {
    const records = parseRawOutput(output);
    if (records.length === 0)
        return null;
    const r = records[0];
 ...
 */
export function parseFoldersWithAccount(output) {
    const records = parseRawOutput(output);
    return records.map((r) => ({
        id: parseNumber(r['id']),
        name: parseString(r['name']),
 ...
 */
export function parseRespondToEventResult(output) {
    const records = parseRawOutput(output);
    if (records.length === 0)
        return null;
 ...
 * @returns Structured result with success status and eventId or error, or null on empty output.
 */
export function parseDeleteEventResult(output) {
    const records = parseRawOutput(output);
    if (records.length === 0)
        return null;
    const record = records[0];
 ...
 */
export function parseUpdateEventResult(output) {
    const records = parseRawOutput(output);
    if (records.length === 0)
        return null;
    const record = records[0];
 ...
 */
export function parseSendEmailResult(output) {
    const records = parseRawOutput(output);
    if (records.length === 0)
        return null;
    const record = records[0];
 ...
 */
export function parseSaveAttachmentResult(output) {
    const records = parseRawOutput(output);
    if (records.length === 0)
        return null;
    const record = records[0];
